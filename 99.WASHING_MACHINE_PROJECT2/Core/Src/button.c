...
//	uint8_t lcd_buff[40];
	...
	if (get_button(BUTTON1_GPIO_Port, BUTTON1_Pin, 1) == BUTTON_PRESS)	// 버튼 1
	{
		lcd_command(CLEAR_DISPLAY);								// for clear from i2c_lcd.c 94
...
/*
		...
		sprintf((char *) lcd_buff, "- Push button0 -");
		move_cursor(1,0);
		lcd_string(lcd_buff);
*/
	}

	if (get_button(BUTTON2_GPIO_Port, BUTTON2_Pin, 2) == BUTTON_PRESS)	// 버튼 2
	{
		lcd_command(CLEAR_DISPLAY);
	}

	if (get_button(BUTTON3_GPIO_Port, BUTTON3_Pin, 3) == BUTTON_PRESS)	// 버튼 3
	{
		lcd_command(CLEAR_DISPLAY);
	}
