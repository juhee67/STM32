...
{
	static int button0_count = 0;					// static 으로 선언하면 전역변수처럼 동작한다. (토글하려면 함수 이전상태 변수 있어야)

	// 버튼을 한 번 눌렀다 뗀 상태라면
	if (get_button(BUTTON0_GPIO_Port, BUTTON0_Pin, 0) == BUTTON_PRESS)						// main.h에서 BUTTON0_GPIO_Port 복사 		dcmotor.c 로 move line-20
	{
		button0_count++;
		button0_count %= 2;
